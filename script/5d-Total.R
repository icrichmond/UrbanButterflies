...
ab_tot_n <- basic_plot(tot_ab, condition = c("avgbloom", "Niche.Breadth"), dat = anp, 
  ...
  annotate("text", x = 20, y = 94, label = bquote("IRR (WA) = " ~ .(round(exp(summary(tot_ab)$coefficients[4,1]), 2)) ~ "+/-" ~ .(round(exp(summary(tot_ab)$coefficients[4,2]), 2)))) + 
  annotate("text", x = 20, y = 90, label = bquote("p-value = " ~ .(round(summary(tot_ab)$coefficients[4,4], 4)))) +
  annotate("text", x = 20, y = 86, label = bquote(R^2 ~ " = "  ~ .(round((with(summary(tot_ab), 1 - deviance/null.deviance)), 2)))) 

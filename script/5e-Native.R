...
# Abundance 
ab_nat <- basic_plot(nat_ab, condition = "nnative", 
...
ab_nat_int <- int_plot(nat_ab, list("avgnatbloom" = 10), condition = c("Niche.Breadth"), 
...
sr_nat <- basic_plot(nat_n_sr, condition = "nnative", 
  ...
  annotate("text", x = 12, y = 10.5, label = bquote("IRR = " ~ .(round(exp(summary(nat_n_sr)$coefficients[2,1]), 2)) ~ "+/-" ~ .(round(exp(summary(nat_n_sr)$coefficients[2,2]), 2)))) + 
  annotate("text", x = 12, y = 10, label = bquote("p-value = " ~ .(round(summary(nat_n_sr)$coefficients[2,4], 2)))) +
  annotate("text", x = 12, y = 9.5, label = bquote(R^2 ~ " = "  ~ .(round((with(summary(nat_n_sr), 1 - deviance/null.deviance)), 2)))) 


#sr_nat_n <- basic_plot(nat_n_sr, condition = c("nnative", "Niche.Breadth"), dat = anp, 
#                       x = nnative, y = Species.Richness, colour = Niche.Breadth,
#                       xlab = "Number of Native Flowering Species", ylab = "Butterfly Species Richness") + 
#  annotate("text", x = 12, y = 10.5, label = bquote("IRR (WA) = " ~ .(round(exp(summary(nat_n_sr)$coefficients[4,1]), 2)) ~ "+/-" ~ .(round(exp(summary(nat_n_sr)$coefficients[4,2]), 2)))) + 
#  annotate("text", x = 12, y = 10, label = bquote("p-value = " ~ .(round(summary(nat_n_sr)$coefficients[4,4], 10)))) +
#  annotate("text", x = 12, y = 9.5, label = bquote(R^2 ~ " = "  ~ .(round((with(summary(nat_n_sr), 1 - deviance/null.deviance)), 2)))) 


nat <- (ab_nat + ab_nat_int) / (sr_nat) + 
...
ggsave('figures/NativeBloom.png', nat, height = 10, width = 10, units = 'in', dpi = 450)

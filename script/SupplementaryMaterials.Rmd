...
```{r, echo = F, eval=T}
var <- read.csv("/home/icrichmond/Repositories/urban-butterflies/input/variables.csv")
...
plant <- read.csv("/home/icrichmond/Repositories/urban-butterflies/output/PlantCleanbySite.csv")
plant <- plant %>% 
  select(nspecies, nnative, avgbloom, avgnatbloom)

dist <- read.csv("/home/icrichmond/Repositories/urban-butterflies/input/SamplingArea.csv")
dist$PerCut[is.na(dist$PerCut)] <- 0 
dist <- dist %>% 
  mutate(Disturbance = if_else(PerCut >= 30, "Disturbed", "Undisturbed"))
Dist <- dist$Disturbance

butt <- read.csv("/home/icrichmond/Repositories/urban-butterflies/output/ButterflyCleanbySite.csv")
butt <- butt %>% 
